 ...
 *
 * $Id: Directory6AgentImpl.java,v 1.1 2006/06/29 18:51:41 akara Exp $
 *
 ...
 */
public class Directory6AgentImpl extends UnicastRemoteObject implements Directory6Agent, Unreferenced {
    ...
    public Directory6AgentImpl() throws RemoteException {
   ...
   */
   public void setup(Run run, String server, String[] instances) throws RemoteException, Exception {
      ...
      for (int i = 0; i < myInstances.length; i++) {
          logger.finer("Agent setting up instance[" + i + "]: " + myInstances[i]);
      }
      // End debug
    ...
    public void start(boolean force, String[] instances) throws RemoteException, IOException {
        ...
        if(instances != null) {
            ...
            for(int i = 0; i < instances.length; i++) {
                int index = instances[i].indexOf('.');
                if (index < 0)
                    instanceHomes[i] = serverHome + File.separator + "slapd-" + instances[i] + File.separator;
                else
                    instanceHomes[i] = serverHome + File.separator + "slapd-" + instances[i].substring(0,index) + File.separator;
            }
        }
        else {
            instanceHomes = new String[myInstances.length];
            for(int i = 0; i < myInstances.length; i++)  {
                int index = myInstances[i].indexOf('.');
                if (index < 0)
                    instanceHomes[i] = serverHome + File.separator + "slapd-" + myInstances[i] + File.separator;
                else
                    instanceHomes[i] = serverHome + File.separator + "slapd-" + myInstances[i].substring(0,index) + File.separator;
            }
        }

        for(int i = 0; i < instanceHomes.length; i++) {
            String cmd = "cd " + instanceHomes[i] + "; ./start-slapd";
    ...
    public void stop(String[] instances) throws RemoteException, IOException {
        ...
        for(int i = 0; i < instanceHomes.length; i++) {
            ...
            String cmd = "cd " + instanceHomes[i] + "; ./stop-slapd";
    ...
    public void xferLogs(String[] instances) throws RemoteException {
        ...
        for(int i = 0; i < instanceHomes.length; i++) {
            ...
            String outFile = outDir + "errors.sjsds6." + instances[i] + "." + CmdAgentImpl.getHost();
